...
type ParsedCommand =
  | { readonly type: "pickup"; readonly truckId: string; readonly loadId: string }
  | {
      readonly type: "travel";
      readonly truckId: string;
      readonly destinationId: string;
    }
  | { readonly type: "deliver"; readonly truckId: string; readonly loadId: string }
...
export function parseCommandInput(value: unknown): ParseOutcome {
  if (!isRecord(value)) return malformed("command");
  ...
  const truckId = requiredString(value, "truckId");
  if (truckId === undefined) return malformed("truckId");

  if (type === "travel") {
    const destinationId = requiredString(value, "destinationId");
    if (destinationId === undefined) return malformed("destinationId");
    return { ok: true, command: { type, truckId, destinationId } };
  }

  const loadId = requiredString(value, "loadId");
  if (loadId === undefined) return malformed("loadId");
...
function malformed(field: string): ParseOutcome {
